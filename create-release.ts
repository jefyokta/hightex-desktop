...
const VERSION_FLAGS = ["--major", "--minor", "--patch"] as const;

type VersionType = (typeof VERSION_FLAGS)[number];

const versionType: VersionType =
  (args.find((arg) => VERSION_FLAGS.includes(arg as VersionType)) as
    | VersionType
    | undefined) ?? "--patch";
...
async function updatePackageVersion(version: string): Promise<void> {
  ...
  await Bun.write(file, JSON.stringify(packageJson, null, 4) + "\n");
}
...
async function mergeCurrentBranchIntoMain(sourceBranch: string): Promise<void> {
  console.log();
...
async function main() {
  ...
  let nextVersion = calculateNextVersion(currentVersion, versionType);
  ...
  if (initialBranch !== MAIN_BRANCH) {
    ...
    nextVersion = calculateNextVersion(mergedVersion, versionType);
  }
  ...
  if (await tagExists(finalTag)) {
    throw new Error(`Git tag "${finalTag}" already exists.`);
  }
  ...
  if (!changedFiles) {
    throw new Error("package.json was not changed.");
  }
