...
type RelationShape<Rel extends Relation> =
  Rel extends HasMany<infer M>        ? UnwrapModel<M>[]  :
  Rel extends BelongsToMany<infer M>  ? UnwrapModel<M>[]  :
  Rel extends HasOne<infer M>         ? UnwrapModel<M>     :
  Rel extends BelongsTo<infer M>      ? UnwrapModel<M>     :
  never;
...
> implements Queryable<T> {
  ...
  protected abstract schema: Record<keyof Omit<T, "id" | "createdAt">, ColumnDefinition>;
  ...
  primaryKey(): Col<T> { return "id" as Col<T>; }
  ...
  readonly boot = (): void => {
    const cols = SchemaBuilder.compile(this.schema, this.primaryKey(), this.primaryKeyType);
    this.connection
  ...
  with<K extends keyof R & string>(
    ...
    if (!relation) throw new Error(`Relation "${relationName}" not defined on ${ctor.name}`);
  ...
  private _applyJoin(relation: Relation): void {
    ...
    if (relation instanceof BelongsTo) {
      ...
      const fk  = relation.foreignId ?? `${this.constructor.name.toLowerCase()}Id`;
  ...
  limit(n: number): this  { this._select.limit(n);  return this; }
  offset(n: number): this { this._select.offset(n); return this; }
  ...
  put(data: T): T | undefined {
    ...
    this._resetBuilders();
    return new (this.constructor as new () => this)().find(id as any) as unknown as T;
  }
...
static with<M extends Model<any, any, any>, K extends keyof RelationsOf<M> & string>(
  this: ModelCtor<M>,
  relationName: K,
  callback?: (query: Select<UnwrapModel<RelationsOf<M>[K]["model"]>>) => void,
): Model<ShapeOf<M>, RelationsOf<M>, WithRelation<ShapeOf<M>, RelationsOf<M>, K>> {
  return new this().with(relationName, callback) ;
}
    
static belongsTo<TModel extends Model<any, any, any>>(
  model: new () => TModel, foreignId?: string, ownerId?: string,
): BelongsTo<TModel> {
...
static belongsToMany<TModel extends Model<any, any, any>>(
  ...
  return new BelongsToMany<TModel>(new model(), pivotTable, pivotForeignKey, pivotRelatedKey);
}
  ...
  get schemaKeys() {
    return [...Object.keys(this.schema), "id", "createdAt"] as (keyof T | "id" | "createdAt")[];
  }
