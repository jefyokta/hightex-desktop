  ...
  const jobs = useMemo<Job[]>(() => {
    ...
    if (inputs.doc.type === "hightex") {
      tasks.push({
        ...
        task: async (_, setProgress: (p: string) => void) => {
          const { id = undefined, title } = (
            inputs.doc as Extract<MainDocument, { type: "hightex" }>
          ).document!;

          if (!id) throw new Error("No Document Selected!");
          setProgress(
            `Compiling document ${truncate(title, 10).replace("_", "")}`,
          );
    ...
    tasks.push({
      ...
      task: async (
        ...
        if (!file) {
          file =
            (inputs.doc as Extract<MainDocument, { type: "pdf" }>).file ||
            undefined;
        }
    ...
    tasks.push({
      ...
      task: async (s: SplitContext, setProgress: (p: string) => void) => {
        ...
        const publicChapters = chapterKeys.filter(
          (_, i) => i <= 2 || i >= chapterKeys.length - 2,
        );
    ...
    <div className="max-w-4xl space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Separate supporting pages from your thesis document before publishing
      ...
      <div className="rounded-2xl border bg-card p-6 shadow-sm space-y-6">
        <div className="rounded-xl border p-5">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-medium">Main Thesis Document</h3>
        ...
        <div className="space-y-4">
          {documents.map((doc) => (
            <div key={doc.key} className="rounded-xl border p-5">
              <div className="flex items-start gap-3">
                <div className="rounded-lg bg-primary/10 p-2">
                  <FileText className="h-5 w-5 text-primary" />
                </div>
