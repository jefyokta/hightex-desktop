...
export class Exporter {
  ...
  async export():Promise<{canceled:boolean,filePath?:string}> {
    ...
    const chaptersContent = await this.getChaptersContent(document.id,document.category)
    ...
    const images = await this.db.images.where("documentId").equals(document.id).toArray();
  ...
  private async getChapterByCategory(categoryId:string){
   ...
   const category= storedCategory.find(c=>c.id.toString() == categoryId) || storedCategory[0]
  ...
  private async getChaptersContent(docId:string,categoryId:string){
    ...
    for(const chapter of chapters){
      ...
      const content =  (await this.db.chapters.get(`${docId}.${chapter}`))?.content
      if (content) {
        tmp.push({
            chapter,
            content
